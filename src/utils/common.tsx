...
export const spreadArrayByShuffledIndex = (originArr, shuffledIndexArr, startIndex=0) => {
  const indexArrByOrigin = shuffledIndexArr.filter(index=>startIndex<=index && index<originArr.length+startIndex)
  return indexArrByOrigin.map(index=>originArr[index])
}
